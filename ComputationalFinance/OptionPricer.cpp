...
double OptionPricer::analytical_solution(Option opt) {
	...
	for (unsigned long j = 0; j < paths; ++j) {
		S_jT = A*exp(B*probability.GetOneGaussianByBoxMuller());
		sum+=(opt)(S_jT);
	}
	...
	while (count < paths) {
		...
		if (opt.contract_type == "plain vanilla") {
			for (unsigned long i = 0; i < bound; ++i) {
				Z = probability.GetOneGaussianByBoxMuller();
...
double* OptionPricer::control_variate(Option opt, double(*method)(double previous, double mesh, double A, double increment, double sigma),double truth) {
	...
	while (count < 1000) {
		...
		if (opt.contract_type == "plain vanilla") {
			for (unsigned long i = 0; i < bound; ++i) {
				S_i = (*method)(S_i, mesh, A, root_mesh*probability.GetOneGaussianByBoxMuller(), opt.sigma); //apply numerical method
			}
		...
		/*UP AND OUT*/
			...
			x[count] = probability.GetOneGaussianByBoxMuller(); //control variate sum
